...
export default function Lobby() {
  ...
  useEffect(() => {
      ...
      if (storedIsHost) {
        ...
        socket.emit('joinRoom', storedRoomId, (response: any) => {
          if (response.success) {
            ...
            // Update player status based on room data
    ...
    socket.on('playerJoined', (data: { roomData: RoomData }) => {
      setRoomData(data.roomData);
      updatePlayerStatus(data.roomData);
    });

    // Listen for another player disconnecting
    socket.on('playerDisconnected', (data: { roomData: RoomData }) => {
      setRoomData(data.roomData);
      updatePlayerStatus(data.roomData);
    });
  ...
  const updatePlayerStatus = (roomData: RoomData) => {
    const host = roomData.players.find(p => p.isHost);
    const guest = roomData.players.find(p => !p.isHost);
    
    setPlayer1Connected(host?.isConnected || false);
    setPlayer2Connected(guest?.isConnected || false);
  };
  ...
  const canStartGame = player1Connected && player2Connected && isHost;
